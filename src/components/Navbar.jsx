                    ...
                    {store.favorites.length === 0 ? (
                        ...
                        store.favorites.map((fav, index) => (
                            <li key={index} className="dropdown-item d-flex justify-content-between align-items-center text-white py-2">
                                {/* Permite navegar al detalle desde el dropdown */}
                                <Link to={`/${fav.type}/${fav.uid}`} className="text-white text-decoration-none text-truncate me-3" style={{ maxWidth: "130px" }}>
                                    {fav.name}
                                </Link>
                                
                                <i className="fas fa-trash-alt text-danger" style={{ cursor: "pointer" }} 
                                   onClick={(e) => { 
                                       e.stopPropagation(); 
                                       dispatch({ type: ACTION_TYPES.deleteFavorite, payload: { uid: fav.uid, type: fav.type } }); 
                                   }}>
                                </i>
                            </li>
                        ))
